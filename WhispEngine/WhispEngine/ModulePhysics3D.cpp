	...
	for(int i = 0; i<numManifolds; i++)
	{
		...
		if(numContacts > 0)
		{
			...
			if(pbodyA && pbodyB && pbodyA->active && pbodyB->active)
			{
				...
				{
					for(auto item = pbodyA->collision_listeners.begin(); item != pbodyA->collision_listeners.end();item++){
						(*item)->OnCollision(pbodyA, pbodyB);
					}

					for (auto item = pbodyB->collision_listeners.begin(); item != pbodyB->collision_listeners.end(); item++) {
...
bool ModulePhysics3D::CleanUp()
{
	...
	for(auto item  = constraints.begin(); item != constraints.end(); ++item)
	{
	...
	for(auto item = shapes.begin(); item != shapes.end(); item++)
		delete *item;

	shapes.clear();

	for(auto item = bodies.begin(); item != bodies.end(); item++)
		delete *item;
